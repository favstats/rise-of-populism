...
ess %>% filter(cntry == "DE") %>% select(cregion) %>% table()
```
...
ess_map %<>% 
  mutate(country = str_extract(id, "^.{2}")) %>% 
  mutate(nuts1 = str_extract(id, "^.{3}")) %>% 
  mutate(nuts2 = str_extract(id, "^.{4}")) %>% 
  mutate(nuts3 = str_extract(id, "^.{5}")) 
...
nuts_data %>% #filter(country == "UK") 
  ...
  arrange(nuts1) %>% #filter(country == "DE") #%>% select(nuts2) %>% table()
  group_by(nuts1) %>% 
  summarise(progressive = mean(progressive, na.rm = T)) %>% 
  mutate(id = nuts1) %>% 
  mutate(progressive = ifelse(is.nan(progressive), NA, progressive)) %>% 
  na.omit() %>% 
  full_join(nuts_data, by = c("nuts1")) %>%  
  select(-nuts1) %>% 
  full_join(fort_map)
```
