...
br1 <- brm(populism2 ~ econ_unsat + econ_insec + unemployed + welfare + 
              anti_imm + age + educ + sex + lrscale + ethnic + religion + 
              rural + globalism + govsat + east + north + south +
              year_2016 + year_2014 + year_2012,
           data=mlogit_ess_adds, family="categorical",
           prior=c(set_prior ("normal (0, 8)")))

save(br1, file = "data/br1.Rdata")
...
plot(marginal_effects(br1))
# 
# br1$ %>% 
#   as.data.frame()

# newdata <- mlogit_ess_adds %>% slice(1)
# predict(br1)
...
br1 %>%
  tidybayes::spread_samples(b_muTraditionalistPopulism_Intercept, b_muTraditionalistPopulism_anti_imm) %>%
  mean_qi(condition_mean = b_muTraditionalistPopulism_Intercept + b_muTraditionalistPopulism_anti_imm) %>%
  ggplot(aes(1, x = condition_mean, xmin = conf.low, xmax = conf.high)) +
  geom_halfeyeh() 

#mlogit_ess_adds %>%
 # modelr::data_grid("b_muTraditionalistPopulism_Intercept") %>%
  add_predicted_samples(mlogit_ess_adds, br1) -> pred_samples
...
```{r}
mlogit_ess_adds %>% #select(target) %>% table 
  #filter(cntry == "Germany") %>% 
  sample_n(3000) -> mlogit_sub

library(brms)

br2 <- brm(populism2 ~ econ_unsat + econ_insec + unemployed + welfare + 
              anti_imm + age + educ + sex + lrscale + ethnic + religion + 
             anti_imm*unemployed +
              rural + globalism + govsat + #east + north + south +
              year_2016 + year_2014 + year_2012,
           data = mlogit_sub, family="categorical",
           prior = c(set_prior ("normal (0, 8)")))

save(br2, file = "data/br2.Rdata")
load("data/br2.Rdata")
...
br2 %>%
  tidybayes::spread_samples(b_muTraditionalistPopulism_anti_imm,
                            b_muTraditionalistPopulism_govsat) %>%
  gather(key, value, -.chain, -.iteration) %>% 
  # mean_qi(condition_mean = b_muTraditionalistPopulism_Intercept +
  #           b_muTraditionalistPopulism_anti_imm) %>%
  ggplot(aes(y = value, x = key)) +
  geom_halfeyeh() 

add_predicted_samples(mlogit_sub, br2) -> pred_samples
add_fitted_samples(mlogit_sub, br2, n = 115) -> fit_samples

library(ggridges)

pred_samples %>%
  ggplot(aes(x = pred, y = anti_imm)) +
  geom_density_ridges()

fit_samples %>%
  ggplot(aes(x = anti_imm, y = estimate, color = category)) +
  geom_smooth(aes(y = estimate, group = paste(category, .iteration)), alpha = 0.25, 
              se = F, size = 0.000000000000025) +
  facet_wrap(~category, scales = "free") +
    ggthemes::theme_hc() +
  ggthemes::scale_colour_gdocs()

ggsave(filename = "images/bayesia_fun.png", width = 10)




sjPlot::plot_model(br2, 
                   terms = c("b_muTraditionalistPopulism_anti_imm",
                             "b_muTraditionalistPopulism_govsat"))
parameters(br2)
```
...
fit_samples %>%
  ungroup() %>% 
  select(govsat, globalism, lrscale, estimate, category, .iteration) %>% 
  gather("variable", "value", -estimate, -.iteration, -category) %>% 
  mutate(variable = 
           case_when(
 ...
 ggplot(aes(x = value, y = estimate, color = category)) +
  geom_smooth(aes(y = estimate, group = paste(category, .iteration)), alpha = 0.25, 
              se = F, size = 0.000000000000025) +
...
```{r}
#add_fitted_samples(mlogit_sub, br2, n = 100) -> fit_samples

fit_samples %>%
  ungroup() %>% 
  select(econ_insec, unemployed, welfare, estimate, category, .iteration) %>% 
  gather("variable", "value", -estimate, -.iteration, -category) %>% 
  mutate(variable =
           case_when(
 ...
 ggplot(aes(x = value, y = estimate, color = category)) +
  geom_smooth(aes(y = estimate, group = paste(category, .iteration)), alpha = 0.25, 
              se = F, size = 0.000000000000025, method = "gam") +
  facet_grid(category~variable, scales = "free") +
    scale_y_continuous(breaks = seq(0,100, by = 5),
                     labels = paste0(seq(0,100, by = 5),"%")) + 
    ggthemes::theme_hc() +
...
ggsave(filename = "slides/images/bayesia_fun3.png", width = 12, height = 9)
```
...
fit_samples %>%
  ungroup() %>% 
  select(anti_imm, religion, age, estimate, category, .iteration) %>% 
  gather("variable", "value", -estimate, -.iteration, -category) %>% 
  mutate(variable =
           case_when(
 ...
 ggplot(aes(x = value, y = estimate, color = category)) +
  geom_smooth(aes(y = estimate, group = paste(category, .iteration)), alpha = 0.25, 
              se = F, size = 0.000000000000025, method = "loess") +
  facet_grid(category~variable, scales = "free") +
    scale_y_continuous(breaks = seq(0,100, by = 5),
                     labels = paste0(seq(0,100, by = 5),"%")) + 
    ggthemes::theme_hc() +
...
ggsave(filename = "slides/images/bayesia_fun4.png", width = 12, height = 9)
```
...
fit_samples %>%
  ungroup() %>% 
  select(unemployed, anti_imm, estimate, category, .iteration) %>% 
  #gather("variable", "value", -estimate, -.iteration, -category, -unemployed) %>% 
  # mutate(variable = 
  #          case_when(
  #            variable == "unemployed" ~ "Unemployment (0/1)",
  #            variable == "anti_imm" ~ "Anti-Immigration Sentiment"
  # mutate(category = 
  #          case_when(
  #            category == "1" ~ "Establishment",
  ...
  #          )) %>% 
mutate(estimate = round(estimate * 100, 2)) %>% 
 ggplot(aes(x = anti_imm, y = estimate, color = unemployed)) +
  geom_smooth(aes(y = estimate, group = paste(unemployed, .iteration)), alpha = 0.25, 
              se = F, size = 0.000000000000025) +
