...
ches_final %>% 
  ...
  select(cntry, party_name, party_cntry, populism, prog_trad, cluster) %>% 
  mutate(party_name = str_remove(party_name, ";")) %>% 
  mutate(party_name = str_replace(party_name, "<U+021B>", " ț")) %>%  
  mutate(party_name = str_replace(party_name, "<U+0219>", "ș")) %>% 
  mutate(party_name = case_when(
    str_detect(party_name, "formateur") ~ "Parti Réformateur Libéral",
    str_detect(party_name, "Parti Social Chr") ~ "Parti Social Chrétien",
    str_detect(party_name, "mocratique des Francophones") ~ "Front Démocratique des Bruxellois Francophones",    
    str_detect(party_name, "Partij van de Arbeid van Belgi") ~ "Partij van de Arbeid van België", 
    str_detect(party_name, "gelsen mod EU") ~ "Folkebevægelsen mod EU",  
    ...
    str_detect(party_name, "lne demokratick") ~ "Česká strana sociálně demokratická",  
    str_detect(party_name, " Demokratick") ~ "Občanská demokratická strana",  
    ...
    str_detect(party_name, "Saskanas Centrs") ~ "Sociāldemokrātiskā Partija", 
    str_detect(party_name, "Latvijas Re") ~ "Latvijas Reģionu Apvienība",  
    str_detect(party_name, "Lietuvos socialdemokratu") ~ "Lietuvos Socialdemokratų Partija",  
    str_detect(party_name, "Tevynes sajunga") ~ "Tėvynės sąjunga",  
    str_detect(party_name, "Lietuvos valstieciu") ~ "Lietuvos valstiečių ir žaliųjų sąjunga",  
    str_detect(party_name, "Lietuvos lenku") ~ "Lietuvos lenkų rinkimų akcija",  
    str_detect(party_name, "Lietuvos Respublikos Liberalu") ~ "Liberalų Sąjūdis",  
    str_detect(party_name, "Ruch Palikota") ~ "Ruch Palikota",  
    str_detect(party_name, "ionala pentru Progresul Rom") ~ "Uniunea Națională pentru Progresul României",  
   ...
   str_detect(party_cntry, "PMP") ~ "Partidul Mișcarea Populară", 
    TRUE ~ party_name
  )) %>% 
